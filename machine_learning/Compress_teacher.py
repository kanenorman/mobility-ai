...
def prune_and_fine_tune(best_model, data, config, pruning_params, is_neuron_pruning=False):
    """
    ...
    if is_neuron_pruning:
        pruned_model = tfmot.sparsity.keras.prune_low_magnitude(pruned_model, pruning_schedule=pruning_schedule)
    else:
        pruned_model = tfmot.sparsity.keras.prune_low_magnitude(pruned_model, pruning_schedule=pruning_schedule)
    ...
    fine_tuning_history = pruned_model.fit(
        ...
        verbose=2,
        callbacks=[DNN_early_10, DNN_plateau_lr, DNN_BestFit, pruning_callback]  # pruning_callback (necessary)
    )

    # Save pruned model
    pruned_model.save(pruned_model_save_path)
    print(f"Pruned model saved to: {pruned_model_save_path}") #Report save location for reference
...
pruned_weight_model = prune_and_fine_tune(best_trained_dnn_model_copy, mbta_final_df, best_hyperparameters, weight_pruning_params, is_neuron_pruning=False)

# Create another copy of the best model for neuron pruning (best to keep separate)
best_trained_dnn_model_copy = tf.keras.models.clone_model(best_trained_dnn_model)

# Perform neuron pruning
pruned_neuron_model = prune_and_fine_tune(best_trained_dnn_model_copy, mbta_final_df, best_hyperparameters, neuron_pruning_params, is_neuron_pruning=True)
